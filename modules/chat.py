...
class Chat:
  ...
  def regen_msg(self, tau, lr, lr_decay, top_p, temp, presence_penalty):
    ...
    chat_param = self.model_utils.format_chat_param(tau, lr, lr_decay, top_p, temp, presence_penalty)
    reply_text = self.__gen_msg(out, chat_param, model_tokens, model_state) 
    return '', reply_text
  
  def on_message(self, message, tau, lr, lr_decay, top_p, temp, presence_penalty, replace_message):
    if self.chunked_index:
      self.__flush_chat()
  ...
  def get_prompt(self, tau, lr, lr_decay, top_p, temp, presence_penalty):
    if self.chunked_index:
      self.__flush_chat()
    out, model_tokens, model_state = self.model_utils.load_all_stat('chat')
