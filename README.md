...
这次更新中加入了一个最小回复长度的参数，可以用这个方法让AI回复的更长（比调大“生成次数”控制的更好一些，但是有可能输出很多无用的废话），如果和“生成次数”一起使用的话，得到的生成最小值依然是该参数值，而不是生成次数x最小回复长度。

更新中加入了一个“生成次数”的配置参数，将该参数调大后，程序会尝试让模型生成更长的回复文本，当然了，内容也更不可控，有时候会跑偏，甚至出现格式错误的问题，可能需要多重试几次，个人建议这个参数不要超过5。
...
2. 换成这种格式之后，角色回复会明显变短，这时可以尝试着把“生成次数”或者“最小回复长度”这些参数给加大，从而得到更长的回复，也更容易推动对话，但是我不建议在任何情况下都把这些参数给加大，而是应该在角色的回复过短导致剧情推不下去的情况下使用，否则会导致角色回复的内容跑题严重。
