...
- TAU：此为Mirostat采样的参数，建议设为3，这个值有点儿像temperature，设置高了容易说胡话，不过辅以Min P，调高了也没啥问题。
- Learning Rate：这个值我找了半天，也没看到有啥推荐的方案，它的大小决定达到目标值的速度快慢，可以保持默认值，设为0.1。
- Min P：候选token中最高的那个token的概率乘以Min P的值，丢弃掉概率小于该结果的token，这个值越大，输出的结果就越准确，但生成的内容也更容易重复。
- 动态温度最小值：参考了一些别人的代码，我把温度（temperature）改成基于信息熵的动态调整了，温度会在最小值和接下来的最大值之间，根据候选词熵的情况自动进行调整。
- 动态温度最大值：同上，这是个最大值。
- 动态温度指数：这个指数其实就是在算出来候选词的信息熵的基础上，再进行一步调整，该值越小，调整的幅度就越大，为1则不调整。
