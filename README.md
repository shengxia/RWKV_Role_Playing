...
还玩个屁的Glow，RWKV-4-World-CHNtuned-3B-v1-20230625-ctx4096这个模型效果好的出奇，3B的参数量就能扮演一个不错的赛博小女友（我特么都不敢想出了7B是啥效果），玩了几天我都要喝营养快线了，现在7B的也出来了（RWKV-4-World-CHNtuned-7B-v1-20230709-ctx4096），效果确实强。
...
最近的更新中加入了一个最小回复长度的参数，可以用这个方法让AI回复的更长（但是有可能输出很多无用的废话）。同时增加了替AI说的功能，当始终得不到符合心意的回复时，可以帮AI说一句，这样更有利于让剧情按照你的意思发展。
...
我个人比较喜欢让人物在回复时带上动作，所以目前我在代码里面做了一些隐式的定义，当在角色设定中指定了“旁白开始符号”和“旁白结束符号”，且在示例对话中包含这两个符号时，角色的回复每次都会尝试输出动作描写。不过从现在模型的测试效果来看，只要我在示例对话中旁白写的够好，就算我不在代码中进行特殊控制，模型也会在每次回复是输出动作描写，这个我会继续测试，也许在接下来的版本会移除掉这个控制。
...
**我尝试着让这个东西可以让多用户同时使用，但是可能是我的解决方案不好，在重新加载角色时，导致显存不被释放，载入角色次数过多时，就容易出现显存溢出（某人48G的显存都没撑住orz），目前我没有找到什么好的解决方案，于是给支持多用户的那一版放到了multi_user分支，如果需要的话，可以拉取那个分支上的，目前我先把代码切回只支持一人使用的方案。**（话说，有人知道gradio有没有那种……就是……呃……关了网页能给个通知之类的方法吗？）

如果需要多用户的话，可以参考这个项目https://github.com/shengxia/RWKV_Role_Playing_API
