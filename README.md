...
### 7. 为什么重置角色、以及清除上一条对话然后再生成对话时，需要这么长的时间？

最近的更新中，我对一些逻辑进行了调整，在加载角色的时候，如果有.sav文件的话，我会尝试着从.sav文件中读取状态，如果没有的话，就会重新把角色的设定传给模型，这样的好处是，加载角色的时候会很快，那么省下来的时间就放在重置角色了，毕竟我感觉加载角色的时候比重置角色的时候要多；
